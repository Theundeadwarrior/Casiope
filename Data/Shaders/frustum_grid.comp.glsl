...
void main() {
	...
	// Top left point
	screenSpace[0] = vec4( vec2( gl_GlobalInvocationID.x, gl_GlobalInvocationID.y + 1) * TILE_SIZE, 1.0f, 1.0f );
    // Top right point
    screenSpace[1] = vec4( vec2( gl_GlobalInvocationID.x + 1, gl_GlobalInvocationID.y + 1) * TILE_SIZE, 1.0f, 1.0f );
    // Bottom left point
    screenSpace[2] = vec4(  gl_GlobalInvocationID.xy * TILE_SIZE, 1.0f, 1.0f );
    // Bottom right point
    screenSpace[3] = vec4( vec2( gl_GlobalInvocationID.x + 1, gl_GlobalInvocationID.y  ) * TILE_SIZE, 1.0f, 1.0f );
